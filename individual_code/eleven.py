...
def main():
    ...
    ap.add_argument("--region", type=str, default="union_tori", choices=["ellipsoid","torus","union_tori"])
    ap.add_argument("--radii", type=str, default="0.25,0.35,0.75,0.55", help="for ellipsoid")
    ap.add_argument("--rxy1", type=str, default="0.5,0.5", help="circle radii for (x1,y1)")
    ap.add_argument("--rxy2", type=str, default="0.8,0.6", help="circle radii for (x2,y2)")
    ...
    Phi, startB, endB, hist, bestiter, frames = train_min_radius_boundary_R4(
        ...
        polynomial_bound=5e-3,
        report_every=max(5, args.n_iters//12),
        animate=args.anim, max_frames=60
    )

    os.makedirs(args.outdir, exist_ok=True)
    with open(os.path.join(args.outdir,"history.csv"), "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["it","loss","R","rmean","rvar","cx","cy","cp","cq"])
        writer.writeheader()
        for row in hist: writer.writerow(row)

    if args.anim:
        save_projection_animation(frames, outpath=os.path.join(args.outdir,"training.gif"))
    ...
    axes[1].set_title("Projection (x1,y1): mapped")
    fig.tight_layout()
